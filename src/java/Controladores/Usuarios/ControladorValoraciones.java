...
public class ControladorValoraciones extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (vivienda.getCasero().getId().equals(autor.getId())) {
                request.setAttribute("error", "No puedes valorar tu propia vivienda.");
                request.setAttribute("vivienda", vivienda);
                request.getRequestDispatcher("/usuarios/valorarPiso.jsp").forward(request, response);
                return;
            }
            ...
            ex.printStackTrace();
            request.setAttribute("error", "No se pudo guardar la valoración.");
            request.getRequestDispatcher("/usuarios/valorarPiso.jsp").forward(request, response);
            return;
        } finally {
            emf.close();
