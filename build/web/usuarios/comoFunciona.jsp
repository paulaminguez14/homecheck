...
<html>
    ...
    <body>
        ...
        <main style="height: auto; min-height: 90vh;">
            <div class="mainComoFunciona">
                <a href="#" onclick="history.back(); setTimeout(() => location.reload(), 100);" class="enlaceVolverPublicarVivienda" style="color: black;">Volver</a>
                <h1>¿Cómo funciona HomeCheck?</h1>
                <div class="contenedorTarjetasInformacion" id="contenedorInfoJson">
                    <div class="tarjetasInformacion">
                        <h2 class="titulo" data-index="0"></h2>
                        <p class="contenido"></p>
                    </div>

                    <div class="tarjetasInformacion">
                        <h2 class="titulo" data-index="1"></h2>
                        <p class="contenido"></p>
                    </div>

                    <div class="tarjetasInformacion">
                        <h2 class="titulo" data-index="2"></h2>
                        <p class="contenido"></p>
                    </div>

                    <div class="tarjetasInformacion">
                        <h2 class="titulo" data-index="3"></h2>
                        <p class="contenido"></p>
                    </div>
                </div>
            </div>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                fetch('${pageContext.request.contextPath}/usuarios/comoFunciona.json')
                        .then(response => response.json())
                        .then(data => {
                            // data es el array del JSON
                            document.querySelectorAll('.tarjetasInformacion').forEach(function (tarjeta) {
                                const index = tarjeta.querySelector('.titulo').dataset.index;
                                const item = data[index];
                                if (item) {
                                    tarjeta.querySelector('.titulo').textContent = item.titulo;

                                    // Crear párrafos con el contenido
                                    const contenidoDiv = tarjeta.querySelector('.contenido');
                                    contenidoDiv.innerHTML = ''; // limpiar contenido previo
                                    item.contenido.forEach(parrafo => {
